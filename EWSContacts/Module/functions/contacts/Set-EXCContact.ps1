...
#>
	...
	Begin
	{
		...
		if ($Folder)
		{
			if ($Partial)
			{
				$Contacts = Get-EXCContact -MailboxName $MailboxName -EmailAddress $EmailAddress -Credentials $Credentials -Folder $Folder -Partial -useImpersonation
			}
			else
			{ 
				$Contacts = $Contacts = Get-EXCContact -MailboxName $MailboxName -EmailAddress $EmailAddress -Credentials $Credentials -Folder $Folder -useImpersonation
			}
		}
		else
		{
			if ($Partial)
			{
				$Contacts = Get-EXCContact -MailboxName $MailboxName -EmailAddress $EmailAddress -Credentials $Credentials -Partial -useImpersonation
			}
			else
			{
				$Contacts = $Contacts = Get-EXCContact -MailboxName $MailboxName -EmailAddress $EmailAddress -Credentials $Credentials -useImpersonation
			}
		}
		...
		foreach ($Contact in $Contacts)
		{
			if (($Contact.EmailAddresses[[Microsoft.Exchange.WebServices.Data.EmailAddressKey]::EmailAddress1].Address.ToLower() -eq $EmailAddress.ToLower()) -bor $Partial.IsPresent)
			{
				...
				if ($updateOkay)
				{
					...
					if ($CompanyName -ne "" -And $CompanyName -ne $null)
					{
					...
					if ($Department -ne "" -And $Department -ne $null)
					{
